...
from bocchi.utils.github_utils import GithubUtils
from bocchi.utils.http_utils import AsyncHttpx
from bocchi.utils.manager.virtual_env_package_manager import VirtualEnvPackageManager
from bocchi.utils.repo_utils import AliyunRepoManager, GithubRepoManager
from bocchi.utils.repo_utils.models import RepoUpdateResult
from bocchi.utils.repo_utils.utils import check_git

LOG_COMMAND = "bocchiRepoManager"


class bocchiUpdateException(Exception):
    """资源下载异常"""

    pass


class bocchiRepoConfig:
    """波奇仓库配置"""

    # bocchi Bot 相关配置
    bocchi_BOT_GIT = "https://github.com/zhenxun-org/zhenxun_bot.git"
    bocchi_BOT_GITHUB_URL = "https://github.com/HibiKier/zhenxun_bot/tree/main"
    bocchi_BOT_DOWNLOAD_FILE_STRING = "zhenxun_bot.zip"
    bocchi_BOT_DOWNLOAD_FILE = TEMP_PATH / bocchi_BOT_DOWNLOAD_FILE_STRING
    bocchi_BOT_UNZIP_PATH = TEMP_PATH / "zhenxun_bot"
    bocchi_BOT_CODE_PATH = Path() / "zhenxun"
    bocchi_BOT_RELEASES_API_URL = (
        "https://api.github.com/repos/HibiKier/zhenxun_bot/releases/latest"
    )
    bocchi_BOT_BACKUP_PATH = Path() / "backup"
    # 需要替换的文件夹
    bocchi_BOT_UPDATE_FOLDERS: ClassVar[list[str]] = [
        "bocchi/builtin_plugins",
        "bocchi/services",
        "bocchi/utils",
        "bocchi/models",
        "bocchi/configs",
    ]
    bocchi_BOT_VERSION_FILE_STRING = "__version__"
    bocchi_BOT_VERSION_FILE = Path() / bocchi_BOT_VERSION_FILE_STRING
...
class bocchiRepoManagerClass:
    """波奇仓库管理器"""

    def __init__(self):
        self.config = bocchiRepoConfig()
    ...
    async def bocchi_get_version_from_repo(self) -> str:
        """从指定分支获取版本号
        ...
        返回:
            str: 版本号
        """
        repo_info = GithubUtils.parse_github_url(self.config.bocchi_BOT_GITHUB_URL)
        version_url = await repo_info.get_raw_download_urls(
            path=self.config.bocchi_BOT_VERSION_FILE_STRING
        )
        try:
    ...
    async def bocchi_write_version_file(self, version: str):
        """写入版本文件"""
        async with aiofiles.open(
            self.config.bocchi_BOT_VERSION_FILE, "w", encoding="utf8"
        ) as f:
            await f.write(f"__version__: {version}")

    def __backup_bocchi(self):
        """备份波奇文件"""
        for filename in os.listdir(self.config.bocchi_BOT_CODE_PATH):
            file_path = self.config.bocchi_BOT_CODE_PATH / filename
            if file_path.exists():
                self.__copy_files(
                    file_path,
                    self.config.bocchi_BOT_BACKUP_PATH / filename,
                    True,
                )
    ...
    async def bocchi_get_latest_releases_data(self) -> dict:
        """获取波奇releases最新版本信息
        ...
        """
        try:
            res = await AsyncHttpx.get(self.config.bocchi_BOT_RELEASES_API_URL)
            if res.status_code == 200:
                return res.json()
        except Exception as e:
            logger.error("检查更新波奇获取版本失败", LOG_COMMAND, e=e)
        return {}

    async def bocchi_download_zip(self, ver_type: Literal["main", "release"]) -> str:
        """下载波奇最新版文件
        ...
        if ver_type == "main":
            ...
            new_version = await self.bocchi_get_version_from_repo()
        else:
            release_data = await self.bocchi_get_latest_releases_data()
            logger.debug(f"获取波奇RELEASES最新版本信息: {release_data}", LOG_COMMAND)
            if not release_data:
                raise bocchiUpdateException("获取波奇RELEASES最新版本失败...")
            new_version = release_data.get("name", "")
            download_url = await repo_info.get_release_source_download_urls_tgz(
                new_version
            )
        if not download_url:
            raise bocchiUpdateException("获取波奇最新版文件下载链接失败...")
        if self.config.bocchi_BOT_DOWNLOAD_FILE.exists():
            self.config.bocchi_BOT_DOWNLOAD_FILE.unlink()
        if await AsyncHttpx.download_file(
            download_url, self.config.bocchi_BOT_DOWNLOAD_FILE, stream=True
        ):
            logger.debug("下载波奇最新版文件完成...", LOG_COMMAND)
        else:
            raise bocchiUpdateException("下载波奇最新版文件失败...")
        return new_version

    async def bocchi_unzip(self):
        """解压波奇最新版文件"""
        if not self.config.bocchi_BOT_DOWNLOAD_FILE.exists():
            raise FileNotFoundError("波奇最新版文件不存在")
        if self.config.bocchi_BOT_UNZIP_PATH.exists():
            shutil.rmtree(self.config.bocchi_BOT_UNZIP_PATH)
        tf = None
        try:
            tf = zipfile.ZipFile(self.config.bocchi_BOT_DOWNLOAD_FILE)
            tf.extractall(self.config.bocchi_BOT_UNZIP_PATH)
            logger.debug("解压bocchi Bot文件压缩包完成!", LOG_COMMAND)
            self.__backup_bocchi()
            for filename in self.config.BACKUP_FILES:
                self.__copy_files(
                    self.config.bocchi_BOT_UNZIP_PATH / filename,
                    Path() / filename,
                )
            logger.debug("备份波奇更新文件完成!", LOG_COMMAND)
            unzip_dir = next(self.config.bocchi_BOT_UNZIP_PATH.iterdir())
            for folder in self.config.bocchi_BOT_UPDATE_FOLDERS:
                self.__copy_files(unzip_dir / folder, Path() / folder)
            logger.debug("移动波奇更新文件完成!", LOG_COMMAND)
            if self.config.bocchi_BOT_UNZIP_PATH.exists():
                shutil.rmtree(self.config.bocchi_BOT_UNZIP_PATH)
        except Exception as e:
            logger.error("解压波奇最新版文件失败...", LOG_COMMAND, e=e)
            raise
        finally:
            if tf:
                tf.close()

    async def bocchi_zip_update(self, ver_type: Literal["main", "release"]) -> str:
        """使用zip更新波奇
        ...
        返回:
            str: 版本号
        """
        new_version = await self.bocchi_download_zip(ver_type)
        await self.bocchi_unzip()
        await self.bocchi_write_version_file(new_version)
        return new_version

    async def bocchi_git_update(
    ...
    async def bocchi_update(
        ...
        """
        if await check_git():
            await self.bocchi_git_update(source, branch, force)
            logger.debug("使用git更新波奇!", LOG_COMMAND)
        else:
            await self.bocchi_zip_update(ver_type)
            logger.debug("使用zip更新波奇!", LOG_COMMAND)
    ...
    async def resources_download_zip(self):
        ...
        if await AsyncHttpx.download_file(
            ...
            raise bocchiUpdateException("下载resources资源包失败...")
    ...
    async def webui_download_zip(self):
        ...
        if await AsyncHttpx.download_file(
            ...
            raise bocchiUpdateException("下载 WEBUI_ASSETS 失败", LOG_COMMAND)
...
bocchiRepoManager = bocchiRepoManagerClass()
