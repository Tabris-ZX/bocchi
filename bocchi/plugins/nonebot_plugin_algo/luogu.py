...
class Luogu:
    headers = {
            ...
            "x-requested-with": algo_config.luogu_x_requested_with
        }
    ...
    async def get_user_info(cls, user: str| int)-> Dict | None:
        ...
        if user_id is None:
            return None
        url = cls.base_url + f"/user/{user_id}?_contentOnly"
        ...
        if user_info:
            ...
            if prizes:
                user_info['currentData']['user']['prize'] = prizes['prizes']
        return user_info
    ...
    async def build_user_info(cls, user: str|int)-> Path | None:
        ...
        img_output: Path = cards_save_path / f"luogu_{str(user)}.png"
    ...
    def _build_user_card_context(cls, data: Dict) -> Dict:
        """提取用户信息"""
        all_info=data.get("currentData",{})

        #用户部分
        user_info=all_info.get("user",{})
        name = user_info.get("name", "Unknown")
        badge = user_info.get("badge", "")
        color_key = user_info.get("color", "#FFFFFF")
        color = algo_config.luogu_name_color.get(color_key)
        ...
        elo = user_info.get("eloValue") if user_info.get("eloValue") is not None else "--"
        # 徽章文本（若存在则渲染到名字后）
        badge = user_info.get("badge")
        name_badge = ""
        ...
        try:
            for item in prizes:
                ...
                if year or contest or level:
                    ...
                    # 颜色：一等奖(金)#d4af37，二等奖(银)#c0c0c0，三等奖(铜)#cd7f32，其它灰#888
                    level_text = str(level or "")
                    if "一" in level_text:
                        prize_color = "#d4af37"
                    elif "二" in level_text:
                        prize_color = "#c0c0c0"
                    elif "三" in level_text:
                        prize_color = "#cd7f32"
                    else:
                        prize_color = "#888888"
                    prize_rows.append({
        ...
        # 做题情况部分
        problems_info=all_info.get("passedProblems",{})
        passed_problems_counter = Counter(p.get("difficulty") for p in problems_info if isinstance(p, dict))
        ...
        bars = []
        names_map = getattr(algo_config, "luogu_difficulty_names", {}) or {}
