...
__plugin_meta__ = PluginMetadata(
    ...
    description="""
    虽然是运气游戏，但这可是战场啊少年～
    游戏规则：在对决开始后，轮到某玩家的回合，他可以选择向自己或者向对方开一枪，打自己若是空弹则主动权仍在自己，打对方若是空弹则主动权交换。
    最后被实弹击中者判负
    """.strip(),
    usage="""
    又到了决斗时刻
...
# 修改点2：为两个匹配器分别创建处理函数
@_self_shoot_matcher.handle()  # 修改点：使用新的匹配器名称
async def _(bot: Bot, session: EventSession, arparma: Arparma, uname: str = UserName()):
    gid = session.id2
    if not session.id1:
        await MessageUtils.build_message("用户id为空...").finish()
    if not gid:
        await MessageUtils.build_message("群组id为空...").finish()
    # 修改点：添加shoot_type="self"参数表示打自己
    result, settle = await russian_manage.shoot(
        bot, gid, session.id1, uname, session.platform, shoot_type="self"
    )
    await result.send()
    if settle:
        await settle.send()
    logger.info("俄罗斯轮盘开枪(自己)", arparma.header_result, session=session)  # 修改点：日志添加备注

@_opponent_shoot_matcher.handle()  # 修改点：使用新的匹配器名称
async def _(bot: Bot, session: EventSession, arparma: Arparma, uname: str = UserName()):
    gid = session.id2
    if not session.id1:
        await MessageUtils.build_message("用户id为空...").finish()
    if not gid:
        await MessageUtils.build_message("群组id为空...").finish()
    # 修改点：添加shoot_type="opponent"参数表示打对方
    result, settle = await russian_manage.shoot(
        bot, gid, session.id1, uname, session.platform, shoot_type="opponent"
    )
    await result.send()
    if settle:
        await settle.send()
    logger.info("俄罗斯轮盘开枪(对方)", arparma.header_result, session=session)  # 修改点：日志添加备注
