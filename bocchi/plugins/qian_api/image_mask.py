...
async def _(
    ...
    await MessageUtils.build_message("开始抠图了哦，请稍等...").send()
    files = {"file": ("image.jpg", BytesIO(image_data), "image/jpeg")}
    try:
        response = await AsyncHttpx.post(
            "https://api.3000y.ac.cn/v1/image-mask", files=files, timeout=60.0
        )
        ...
        logger.error("抠图发生异常", arparma.header_result, session=session, e=e)
