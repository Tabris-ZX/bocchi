...
from bocchi.utils.github_utils.models import RepoInfo
...
def install_requirement():
    ...
    if not requirement_path.exists():
        logger.debug(
            f"没有找到bocchi的requirement.txt,目标路径为{requirement_path}", COMMAND
        )
        return
    try:
        ...
        logger.debug(f"成功安装波奇依赖，日志:\n{result.stdout}", COMMAND)
    except subprocess.CalledProcessError as e:
        logger.error(f"安装波奇依赖失败，错误:\n{e.stderr}", COMMAND, e=e)
...
class UpdateManager:
    ...
    async def update_webui(cls) -> str:
        from bocchi.builtin_plugins.web_ui.public.data_source import (
            update_webui_assets,
        )
    ...
    async def update(cls, bot: Bot, user_id: str, version_type: str) -> str:
        ...
        返回:
            str | None: 返回消息
        """
        logger.info("开始下载波奇最新版文件....", COMMAND)
        cur_version = cls.__get_version()
        url = None
        new_version = None
        ...
        await PlatformUtils.send_superuser(
            bot,
            f"检测波奇已更新，版本更新：{cur_version} -> {new_version}\n开始更新...",
            user_id,
        )
        download_file = (
        ...
        if await AsyncHttpx.download_file(url, download_file, stream=True):
            logger.debug("下载波奇最新版文件完成...", COMMAND)
            ...
            logger.debug("下载波奇最新版文件失败...", COMMAND)
        return ""
