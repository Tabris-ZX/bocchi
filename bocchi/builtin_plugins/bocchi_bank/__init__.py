...
__plugin_meta__ = PluginMetadata(
    name="小波奇银行",
    description="""
    小波奇银行，提供高品质的存款！当好感度等级达到指初识时，小波奇会偷偷的帮助你哦。
...
async def _(session: Uninfo, arparma: Arparma, amount: Match[int]):
    ...
    if event_rate:
        result += f"（小波奇偷偷将小时利率给你增加了 {event_rate:.2f}% 哦）"
    result += (
        f"\n预计总收益为: {int(amount.result * rate * effective_hour) or 1} 金币。"
    )
    logger.info(
        f"小波奇银行存款:{amount_num},当前存款数:{amount.result},存款小时利率: {rate}",
...
async def _(session: Uninfo, arparma: Arparma, amount: Match[int]):
    ...
    try:
        ...
        logger.info(
            f"小波奇银行取款:{amount_num}, 当前存款数:{user.amount},"
            f" 存款小时利率:{user.rate}",
            arparma.header_result,
            session=session,
        )
...
#         )
#         if event_rate:
#             result += f"（小波奇偷偷将利率给你降低了 {event_rate}% 哦）"
#         result += f"\n预计每小时利息为:{int(user.loan_amount * user.loan_rate)}金币。"
#         logger.info(
#             f"小波奇银行贷款: {amount_num}, 当前贷款数: {user.loan_amount}, "
...
#     logger.info(
#         f"小波奇银行还款:{amount_num},当前贷款数:{user.amount}, 贷款利率:{user.rate}",
#         arparma.header_result,
#         session=session,
#     )
#     await MessageUtils.build_message(result).finish(at_sender=True)
