...
from bocchi.configs.path_config import TEMPLATE_PATH
...
from bocchi.utils.enum import BlockType
...
def build_plugin_data(classify: dict[str, list[Item]]) -> list[dict[str, str]]:
    ...
    for plugin in plugin_list:
        plugin["items"].sort(key=lambda x: int(x.id))
    return plugin_list


async def build_bocchi_image(
